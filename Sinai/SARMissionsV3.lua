...
EasyZone1 = ZONE:FindByName("EasyZone1")
EasyZone2 = ZONE:FindByName("SportVictim")
EasyZone3 = ZONE:FindByName("HikingVictim")
...
function EasyMissionGenerator()
  ...
  EasyZone1:Scan()
  EasyZone1IsEmpty = EasyZone1:IsNoneInZone()
  EasyZone2:Scan()
  EasyZone2IsEmpty = EasyZone2:IsNoneInZone()
  EasyZone3:Scan()
  EasyZone3IsEmpty = EasyZone3:IsNoneInZone()
  if FlagValue = 1 and EasyZone1IsEmpty == true then
  ...
  elseif FlagValue = 2 and EasyZone2IsEmpty == true then
    trigger.action.setUserFlag(easyMissionFlag, FlagValue)--Sets Flag 69 value
    env.info(easyMissionFlag.." Value is "..FlagValue)
    do easymissions(FlagValue)--Runs easymission funciton
  elseif FlagValue = 3 and EasyZone3IsEmpty == true then
    trigger.action.setUserFlag(easyMissionFlag, FlagValue)--Sets Flag 69 value
    env.info(easyMissionFlag.." Value is "..FlagValue)
    do easymissions(FlagValue)--Runs easymission funciton
  else
    return
    trigger.action.outText("Unable to get a mission, all Easy missions may be taken", 15)
