...
function GRIDZONEMAKER() -- Function to make colour changing squares around the MGRS Grids
  for i, gzone in pairs(env.mission.triggers.zones) do
    ...
    if (gzone["properties"] and gzone["properties"][1] and gzone["properties"][1]["value"] == "gridsquare") then -- Looks for the Properties in the mission file. Then picks out Property 1 with the specific value
      ...
      trigger.action.lineToAll(-1,gid,four,three,{192,192,192,0.5} , 5) -- Makes the line, and the colour of the line
      gid = gid+30000
      trigger.action.lineToAll(-1,gid,one,two,{192,192,192,1} , 0.5)
      gid = gid+30000
      trigger.action.lineToAll(-1,gid,four,one,{192,192,192,1} , 0.5)
      gid = gid+30000
      trigger.action.lineToAll(-1,gid,three,two,{192,192,192,1} , 0.5)
    end
  end
end
GRIDZONEMAKER()
