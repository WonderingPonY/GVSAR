...
function CSAR:New(Coalition, Template, Alias)
  ...
  self.csarPrefix = { "helicargo", "TRAINING MEDEVAC"} -- prefixes used for useprefix=true - DON\'T use # in names!
  self.template = Template or "generic" -- template for downed pilot
  self.mashprefix = {"Hospital"} -- prefixes used to find MASHes
  ...
  self.approachdist_far = 5000 -- switch do 10 sec interval approach mode, meters
  self.approachdist_near = 3000 -- switch to 5 sec interval approach mode, meters
  self.pilotmustopendoors = true -- switch to true to enable check on open doors
...
function CSAR:_SpawnPilotInField(country,point,frequency)
  ...
  local alias = string.format(" %.2fkHz-%d", freq, math.random(1,99))
...
function CSAR:_EventHandler(EventData)
  ...
  elseif (_event.id == EVENTS.PilotDead and self.csarOncrash == false) then
      ...
      self:T(self.lid .. " Event unit - Victim Dead")
      ...
      if self.takenOff[_event.IniUnitName] == true or _group:IsAirborne() then
          ...
          self:T(self.lid .. " Victim has not taken off, ignore")
      end
...
function CSAR:_PickupUnit(_heliUnit, _pilotName, _woundedGroup, _woundedGroupName)
  ...
  self:_DisplayMessageToSAR(_heliUnit, string.format("%s: %s I\'m in! Get to the Hospital ASAP! ", _heliName, _pilotName), self.messageTime,true,true)
...
function CSAR:_RescuePilots(_heliUnit)
  self:T(self.lid .. " _RescueVictims")
  local _heliName = _heliUnit:GetName()
  ...
  local _txt = string.format("%s: The %d Victim(s) have been taken to the\nmedical clinic. Good job!", _heliName, PilotsSaved)
...
function CSAR:_DisplayActiveSAR(_unitName)
  ...
  for _, _value in pairs(_DownedPilotTable) do
    ...
    self:T(string.format("Display Active Victim: %s", tostring(_groupName)))
    self:T({Table=_value})
...
function CSAR:_SignalFlare(_unitName)
  ...
  if _closest ~= nil and _closest.pilot ~= nil and _closest.distance < smokedist then
      ...
      self:_DisplayMessageToSAR(_heli, string.format("No Victims within %s",_distance), self.messageTime)
  end
  return self
end
...
function CSAR:_CheckOnboard(_unitName)
  self:T(self.lid .. " _CheckOnboard")
    ...
    if _inTransit == nil then
        self:_DisplayMessageToSAR(_unit, "No Rescued Victims onboard", self.messageTime)
    else
...
function CSAR:onafterStatus(From, Event, To)
  ...
  if self.verbose > 0 then
    local text = string.format("%s Active SAR: %d | Downed Victims in field: %d (max %d) | Victims boarded: %d | Landings: %d |  s rescued: %d",
